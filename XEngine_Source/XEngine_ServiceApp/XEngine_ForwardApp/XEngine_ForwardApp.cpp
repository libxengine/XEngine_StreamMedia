...
int main(int argc, char** argv)
{
	...
	if (st_ForwardConfig.nHttpPort > 0)
	{
		...
		XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _X("启动服务中,初始化HTTP组包成功,IO线程个数:%d"), st_ForwardConfig.st_XMax.nHTTPThread);
		//启动心跳
		if (st_ForwardConfig.st_XTime.nHTTPTimeOut > 0)
		{
			...
			XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _X("启动服务中,初始化HTTP心跳服务成功,句柄:%llu,时间:%d,次数:%d"), xhHttpHeart, st_ForwardConfig.st_XTime.nHTTPTimeOut, st_ForwardConfig.st_XTime.nTimeCheck);
		}
		else
		{
		...
		XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _X("启动服务中,启动HTTP网络服务器成功,HTTP端口:%d,IO:%d"), st_ForwardConfig.nHttpPort, st_ForwardConfig.st_XMax.nIOThread);
		...
		XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _X("启动服务中,启动HTTP线程池服务成功,启动个数:%d"), st_ForwardConfig.st_XMax.nHTTPThread);
	}
	else
	{
		XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_WARN, _X("启动服务中,HTTP消息服务没有被启用"));
	}
	XLOG_PRINT(xhLog, XENGINE_HELPCOMPONENTS_XLOG_IN_LOGLEVEL_INFO, _X("所有服务成功启动,服务运行中,XEngine版本:%s,服务版本:%s,发行次数;%d。。。"), BaseLib_OperatorVer_XNumberStr(), st_ForwardConfig.st_XVer.pStl_ListVer->front().c_str(), st_ForwardConfig.st_XVer.pStl_ListVer->size());
