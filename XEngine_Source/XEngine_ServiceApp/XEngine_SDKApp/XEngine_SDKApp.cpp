...
int main(int argc, char** argv)
{
	...
	for (auto stl_ListIterator = st_SDKConfig.st_XPlugin.pStl_ListPlugin->begin(); stl_ListIterator != st_SDKConfig.st_XPlugin.pStl_ListPlugin->end(); stl_ListIterator++)
	{
		if (stl_ListIterator->bEnable)
		{
			...
			if (!ModulePlugin_Core_Init(stl_ListIterator->xhToken, stl_ListIterator->tszPluginAddr, stl_ListIterator->nPort, stl_ListIterator->tszPluginUser, stl_ListIterator->tszPluginPass, st_SDKConfig.bPacket, st_ServiceConfig.bDebug))
			{
